...
SendMessageToUser(userName) {
    ...
    ; Slack 창 활성화
    UpdateProgressBar(currentStep * 100 / totalSteps, "Slack 창을 활성화하는 중...")
    ...
    currentStep++
    UpdateProgressBar(currentStep * 100 / totalSteps, "대화 또는 채널 검색 중...")
    if (shouldCancelUpload) {
    return
    }
...
; ============================================================================
; 파일 첨부 함수 - 프로그레스 바 표시하며 파일 첨부
; ============================================================================
AttachFiles() {
    global attachFile, filePathCount, filePathList
    global isUploading, shouldCancelUpload, currentStep, totalSteps
    
    ; 파일 첨부 체크박스가 선택되지 않았으면 리턴
    if (!attachFile) {
        ; 파일 첨부하지 않더라도 프로그레스 바 완료 처리
