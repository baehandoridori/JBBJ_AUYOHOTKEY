...
; (D) 결과창 - 리치에디트 컨트롤 사용
Gui, 2: Add, Edit, x52 y59 w370 h590 vResultEdit +ReadOnly
...
; 숫자 버튼 HWND 수집
global hBtn := {}
For i, v in ["Btn0","Btn1","Btn2","Btn3","Btn4","Btn5","Btn6","Btn7","Btn8","Btn9"]
{
...
Loop, Parse, userInput
{
    ...
    else ; 볼인 경우
    {
        ...
        if (digitStatusMap[currentDigit] != "STRIKE")
...
UpdateResultDisplay()
{
    global tryHistory, digitStatusMap
    
    GuiControlGet, hintModeActive, 2:, HintMode
    newResultText := ""
    
    ; 모든 시도 결과 표시
    for idx, try in tryHistory
    {
        newLine := "시도 " . idx . ": "
        
        ; 힌트 모드일 때는 각 숫자에 대한 상태를 텍스트로 표시
        if (hintModeActive)
        {
            Loop, Parse, % try.input
            {
                currentDigit := A_LoopField
                pos := InStr(secretNumber, currentDigit)
                ...
                if (pos = 0) ; 아웃
                {
                    newLine .= "[" . currentDigit . " 아웃] "
                }
                else if (pos = A_Index) ; 스트라이크
                {
                    newLine .= "[" . currentDigit . " 스트라이크] "
                }
                else ; 볼
                {
                    newLine .= "[" . currentDigit . " 볼] "
                }
            }
        }
        else
        ...
        newLine .= " => " . try.strike . " S, " . try.ball . " B, " . try.out . " O`r`n"
        newResultText .= newLine
    }
    
    ; 결과창에 표시
    GuiControl, 2:, ResultEdit, %newResultText%
}
...
; ------------------------------------------------------
ToggleHint:
GuiControlGet, ht_hintState, 2:, HintMode
if (ht_hintState = 1)
    hintUsed := true
...
UpdateHintButtons()
{
    ...
    if (uhb_hintState = 1)
    {
        ...
        {
            if (digitStatusMap.HasKey(btnDigit))
            {
                ...
                if (btnStatus = "STRIKE")
                {
                    ...
                    GuiControl, 2: +BackgroundFFD700, Btn%btnDigit%
                    ; 눌림 효과 설정
                ...
                {
                    ; 볼(숫자만 일치): 초록색 (32CD32)
                    GuiControl, 2: +Background32CD32, Btn%btnDigit%
                    ; 눌림 효과 설정
                    SendMessage, 0xF3, 1, 0,, ahk_id %btnHwnd%  ; BM_SETSTATE = 0xF3
                }
                else if (btnStatus = "OUT")
                {
                    ...
                    GuiControl, 2: +BackgroundB22222, Btn%btnDigit%
                    ; 눌림 효과 해제
            ...
            else
            {
                ...
                GuiControl, 2: +Background, Btn%btnDigit%
                ; 눌림 효과 해제
                SendMessage, 0xF3, 0, 0,, ahk_id %btnHwnd%
            }
            
            ; 버튼 다시 그리기
            GuiControl, 2: MoveDraw, Btn%btnDigit%
        }
    }
    else
    {
...
RestartGame()
{
    global digitCount, tryCount, startTime, hintUsed, digitStatusMap, hBtn, tryHistory
