...
; Edit 컨트롤을 ListBox로 변경하여 색상 코드를 클릭할 수 있도록 함
Gui, Add, ListBox, x12 y79 w140 h160 vColorHistory gColorHistoryClick

Gui, Add, Progress, x162 y79 w100 h90 vColorSample -Smooth, 100
Gui, Add, Progress, x162 y179 w100 h90 vSelectedColorSample -Smooth, 100  ; 선택한 색상 표시
...
; 마우스 클릭 이벤트 (Ctrl + Left Click 시 색상 복사 및 일시정지)
#If isActive
~^LButton::
...
GuiControl,, SelectedColorSample, 0
return
...
AddColorToHistory(colorCode) {
    GuiControlGet, currentHistory, , ColorHistory, |  ; ListBox에서 항목 가져오기
    if (currentHistory = "")
        newHistory := "#" . colorCode
    else
        newHistory := "#" . colorCode . "|" . currentHistory
    GuiControl,, ColorHistory, |%newHistory%  ; 리스트 갱신
}

; 색상 내역에서 색상 코드를 클릭했을 때
ColorHistoryClick:
GuiControlGet, SelectedColor,, ColorHistory
; '#' 문자 제거
selectedColorCode := SubStr(SelectedColor, 2)
; 선택한 색상 표시 업데이트
GuiControl, +c%selectedColorCode%, SelectedColorSample
GuiControl,, SelectedColorSample, 100
return
