...
mohoObj["SetupPath"] := "G:\\공유 드라이브\\JBBJ 자료실\\애니메이션 관련 자료들\\Moho1430_Win.exe"
mohoObj["CheckFunc"] :=  Func("DefaultCheck")
...
; -----------------------------------------------------------------------------
; [2] GUI 생성 및 표시
;    - 아래 루프(for index, prog)에서 각 프로그램별 컨트롤을 자동 생성합니다.
; -----------------------------------------------------------------------------
Gui, Destroy
Gui, +Resize
Gui, Font, s10
...
{
    ; 1) 각 GUI 컨트롤 이름(변수명) 만들기
    installedCtrlName    := "installedVar" . index
    notInstalledCtrlName := "notInstalledVar" . index
    buttonCtrlName       := "buttonVar"     . index

    ; 2) 프로그램 객체에 '이 컨트롤 이름들'을 기록해둠 (나중에 GuiControl 시 사용)
    prog.installedControl := installedCtrlName
    prog.notInstalledControl := notInstalledCtrlName
    prog.buttonControl := buttonCtrlName
    ...
    ; 5) [설치 진행] 버튼 (검정색)
    Gui, Font, s10 cBlack
    Gui, Add, Button, x+10 w70 v%buttonCtrlName% gInstallProgram Hidden
        , 설치 진행
...
DefaultCheck(index, prog)
{
    ...
    {
        GuiControl,, %installedCtrl%, % "[" . prog.Name . "] - 설치되어 있습니다."
...
CheckSWMoho(index, prog)
{
    ...
    if (startFile = "")
    {
        GuiControl,, %notInstalledCtrl%, % "[" . prog.Name . "] - (파일 없음) 설치되어 있지 않습니다."
        GuiControl, Show, %notInstalledCtrl%
        GuiControl, Show, %buttonCtrl%
        GuiControl, Hide, %installedCtrl%
    }
    else
    {
        ...
        {
            GuiControl,, %notInstalledCtrl%, % "[" . prog.Name . "] - (구버전:" . startVer . ") 더 높은 버전:" . sourceVer . " 존재"
            GuiControl, Show, %notInstalledCtrl%
            GuiControl, Show, %buttonCtrl%
            GuiControl, Hide, %installedCtrl%
        }
        ...
        {
            GuiControl,, %installedCtrl%, % "[" . prog.Name . "] - 설치됨 (현재:" . startVer . ")"
            GuiControl, Show, %installedCtrl%
            GuiControl, Hide, %notInstalledCtrl%
            GuiControl, Hide, %buttonCtrl%
        }
...
CheckBlenderByFolder(index, prog)
{
    ...
    if (foundMsi = "")
    {
        GuiControl,, %installedCtrl%, % "[" . prog.Name . "] - 설치됨 (현재:" . foundFolderVer . ")"
        GuiControl, Show, %installedCtrl%
        GuiControl, Hide, %notInstalledCtrl%
        GuiControl, Hide, %buttonCtrl%
    }
    else
    {
        ...
        {
            GuiControl,, %notInstalledCtrl%, % "[" . prog.Name . "] - 구버전:" . foundFolderVer . " / 새버전:" . foundMsiVer
            GuiControl, Show, %notInstalledCtrl%
            GuiControl, Show, %buttonCtrl%
            GuiControl, Hide, %installedCtrl%
        }
...
InstallSWMoho(index, prog)
{
    ; 예) 자료실에서 파일을 복사하는 로직 추가
    ; FileCopy, %sourceFile%, %prog.InstallPath%, 1
