...
global fileShareState := 1  ; 기본값 켜짐
global autoIMEState := 1    ; 기본값 켜짐
global easyOpenState := 1   ; 기본값 켜짐
global alwaysOnTopState := 1 ; 기본값 켜짐
...
; 체크박스 부분을 버튼으로 변경
Gui, Add, Custom, ClassButton x352 y99 w150 h30 vFileShareBtn hwndHFileShare gFileShareToggle, 파일공유_JBBJ
Gui, Add, Custom, ClassButton x352 y129 w150 h20 vAutoIMEBtn hwndHAutoIME gAutoIMEToggle, 자동 한영전환
Gui, Add, Custom, ClassButton x352 y149 w150 h20 vEasyOpenBtn hwndHEasyOpen gEasyOpenToggle, 경로 쉽게열기
Gui, Add, Custom, ClassButton x352 y169 w150 h20 vAlwaysOnTopBtn hwndHAlwaysOnTop gAlwaysOnTopToggle, AlwaysOnTop
...
OnMessage(0x0F, "WM_PAINT")       ; 버튼 색상 변경용

WM_PAINT(wParam, lParam, msg, hwnd) {
    global HFileShare, HAutoIME, HEasyOpen, HAlwaysOnTop
    global fileShareState, autoIMEState, easyOpenState, alwaysOnTopState
    
    if (hwnd = HFileShare)
        SetButtonColor(hwnd, fileShareState)
    else if (hwnd = HAutoIME)
        SetButtonColor(hwnd, autoIMEState)
    else if (hwnd = HEasyOpen)
        SetButtonColor(hwnd, easyOpenState)
    else if (hwnd = HAlwaysOnTop)
        SetButtonColor(hwnd, alwaysOnTopState)
}

SetButtonColor(hwnd, state) {
    ; DllCall을 사용하여 버튼 텍스트 색상 변경
    if (state)
        DllCall("SendMessage", "Ptr", hwnd, "UInt", 0x0128, "Ptr", 0, "Ptr", 0x00FF00)  ; 초록색
    else
        DllCall("SendMessage", "Ptr", hwnd, "UInt", 0x0128, "Ptr", 0, "Ptr", 0xFF0000)  ; 빨간색
}
...
FileShareToggle:
    ...
    InvalidateRect(HFileShare)
return

AutoIMEToggle:
    autoIMEState := !autoIMEState
    checkAutoIME := autoIMEState
    InvalidateRect(HAutoIME)
return

EasyOpenToggle:
    easyOpenState := !easyOpenState
    checkEasyOpen := easyOpenState
    InvalidateRect(HEasyOpen)
return

AlwaysOnTopToggle:
    alwaysOnTopState := !alwaysOnTopState
    checkAlwaysOnTop := alwaysOnTopState
    InvalidateRect(HAlwaysOnTop)
return
...
InvalidateRect(hwnd) {
    ...
    DllCall("InvalidateRect", "Ptr", hwnd, "Ptr", &rect, "Int", 1)
}
...
WM_MOUSEMOVE(wParam, lParam, msg, hwnd) {
    ...
    if (hwnd != prevHwnd) {
        ...
        if (hwnd = HFileShare)
            ToolTip, 단축키: 파일공유 프로그램 내부 설명서를 참고하세요
        else if (hwnd = HAutoIME)
            ToolTip, 단축키: 마우스를 움직일 때 자동으로 영문으로 전환됩니다
        else if (hwnd = HEasyOpen)
            ToolTip, 단축키: CapsLock 키 더블 클릭
        else if (hwnd = HAlwaysOnTop)
            ToolTip, 단축키: Alt + `
        else
