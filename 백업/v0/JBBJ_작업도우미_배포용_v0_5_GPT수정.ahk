...
; ------------------------------------------------------------------------
; ★ 기능 토글용 그룹박스 + 버튼들 (수정: 시각적 토글 구현)
; ------------------------------------------------------------------------
Gui, Font, S10, Verdana
Gui, Add, GroupBox, x350 y69 w100 h130 , 기능 토글
Gui, Font, cGreen

Gui, Font, S7, Verdana

; [파일공유_JBBJ] 버튼 - 수정: +0x4(BS_CHECKBOX) +0x1000(BS_PUSHLIKE) & 기본 Checked
Gui, Add, Button, x360 y85 w80 h22 hwndHFileShare vBtnFileShare gToggleFileShare +0x4 +0x1000, 파일공유_JBBJ
GuiControl, +Checked, BtnFileShare

; [자동 한영전환] 버튼 - 수정
Gui, Add, Button, x360 y110 w80 h22 hwndHAutoIME vBtnAutoIME gToggleAutoIME +0x4 +0x1000, 자동 한영전환
GuiControl, +Checked, BtnAutoIME

; [경로 쉽게열기] 버튼 - 수정
Gui, Add, Button, x360 y135 w80 h22 hwndHEasyOpen vBtnEasyOpen gToggleEasyOpen +0x4 +0x1000, 경로 쉽게열기
GuiControl, +Checked, BtnEasyOpen

; [AlwaysOnTop] 버튼 - 수정
Gui, Add, Button, x360 y160 w80 h22 hwndHAlwaysOnTop vBtnAlwaysOnTop gToggleAlwaysOnTop +0x4 +0x1000, AlwaysOnTop
GuiControl, +Checked, BtnAlwaysOnTop
...
; ------------------------------------------------------------------------
; [ToggleFileShare] - 수정: 버튼 체크 상태 읽기
; ------------------------------------------------------------------------
ToggleFileShare:
{
    GuiControlGet, fileShareState,, BtnFileShare
    if (fileShareState) {
        FileShareChecked := 1
        Gui, Font, cGreen
        if (fileSharePID = 0) {
    ...
    GuiControl, MoveDraw, BtnFileShare
    Gui, Font, cDefault
}
return
...
ToggleAutoIME:
{
    GuiControlGet, autoIMEState,, BtnAutoIME
    if (autoIMEState) {
        checkAutoIME := 1
        Gui, Font, cGreen
    } else {
    ...
    GuiControl, MoveDraw, BtnAutoIME
    Gui, Font, cDefault
}
return
...
ToggleEasyOpen:
{
    GuiControlGet, easyOpenState,, BtnEasyOpen
    if (easyOpenState) {
        checkEasyOpen := 1
        Gui, Font, cGreen
    } else {
    ...
    GuiControl, MoveDraw, BtnEasyOpen
    Gui, Font, cDefault
}
return

; ------------------------------------------------------------------------
; [ToggleAlwaysOnTop] - 수정
; ------------------------------------------------------------------------
ToggleAlwaysOnTop:
{
    GuiControlGet, alwaysOnTopState,, BtnAlwaysOnTop
    if (alwaysOnTopState) {
        checkAlwaysOnTop := 1
        Gui, Font, cGreen
    } else {
    ...
    GuiControl, MoveDraw, BtnAlwaysOnTop
    Gui, Font, cDefault
}
return
