...
; '전송' 버튼 클릭 시 호출됨
SendFeedback:
    Gui, Feedback:Submit, NoHide  ; GUI 컨트롤의 내용을 변수에 저장 (화면 갱신 없이)
    if (FeedbackText != "" && FeedbackText != PlaceholderText) {  ; 유효한 피드백이 입력되었는지 확인
        feedbackPath := "G:\공유 드라이브\개인작업일지 모음\개인작업일지배한솔\02업무\프로젝트\07_오토핫키 한솔프로젝트\feedback"
        FileAppend, %A_Now%: %FeedbackText%`n, %feedbackPath%\feedback_log.txt  ; 피드백을 파일에 저장
        Gui, Feedback:Destroy  ; 피드백 GUI 닫기
        MsgBox, 262208, 피드백 전송 완료, 피드백이 전송되었습니다. 감사합니다!  ; 완료 메시지 표시 (항상 최상위)
        ExitApp  ; 스크립트 종료
    } else {
        MsgBox, 262160, 입력 오류, 피드백 내용을 입력해주세요.  ; 오류 메시지 표시 (항상 최상위)
    }
return
