...
global checkEDPS := 1            ; EDPS 기능 토글 (기본 on)

; --- 추가: 툴팁용 전역 핸들 변수 (각 버튼에 대한 hWnd) ---
global HFileShare, HAutoIME, HEasyOpen, HAlwaysOnTop, HEDPS, HSvg, Hfeedback, HColor, HAp, Hsnake, H2048, Hmenuchcun, Hfortune, Hcutnumber, Hhelp, Hsetup
...
;Gui, Add, GroupBox, x12 y69 w330 h160 , 기본 기능

;Gui, Font, S8 CDefault, Verdana
;Gui, Add, Text, x22 y119 w310 h50 , 단축키를 입력할 때 한글로 입력되는 경우를 방지해 줍니다. 지원 프로그램 목록을 참고해주세요.
;Gui, Add, Text, x22 y179 w310 h40 , 경로 선택 후 Caps Lock 키를 두번 누르면 해당 경로가 파일 탐색기에 열립니다.

;Gui, Font, S13 Cgreen Bold, Verdana
;Gui, Add, Text, x22 y89 w110 h30 , 자동 한/영전환
;Gui, Add, Text, x22 y150 w150 h30 , 파일 경로 쉽게열기

; (초기 설정 버튼 + 지원 프로그램 목록 버튼)
;Gui, Font, S8 norm, Verdana
;Gui, Add, Button, x132 y85 w80 h30 gInitialSetup, 초기 설정
;Gui, Add, Button, x212 y85 w120 h30 gShowSupportedPrograms, 지원 프로그램 목록
...
; = [EDPS] 버튼
Gui, Add, Button, x360 y185 w80 h22 hwndHEDPS vBtnEDPS +%BS_PUSHLIKE%, 개발중
...
Gui, Add, Text, x400 y39 w90 h30 , ver_1_7
...
WM_MOUSEMOVE(wParam, lParam, msg, hwnd) {
    ...
    if (hwnd != prevHwnd) {
        ...
        if (hwnd = HFileShare || hwnd = HAutoIME || hwnd = HEasyOpen || hwnd = HAlwaysOnTop || hwnd = HEDPS || hwnd = Hsvg || hwnd = Hfeedback || hwnd = HColor || hwnd = Hsetup || hwnd = Hcutnumber || hwnd = Hhelp || hwnd = HAp || hwnd = Hsnake || hwnd = H2048 || hwnd = Hmenuchcun || hwnd = Hfortune)
...
!`::
   ...
   ; ----------------------------------------------------------------------
   ; 수정: moho.exe (ahk_class LM_Wnd) 활성화 시 Alt+ 핫키 무시
   ; ----------------------------------------------------------------------
   if WinActive("ahk_class LM_Wnd")
       return
...

{
    ...
    if (idleTime >= AFK_THRESHOLD) { 
        ...
        ; 특별히 AutoHotkey64.exe 프로세스 처리
        if (currentProcess = "AutoHotkey64.exe" || currentProcess = "AutoHotkey.exe") {
            WinGetTitle, winTitle, ahk_id %currentHwnd%
            
            ; 타이틀이 있고 의미 있는 경우만 처리
            if (winTitle && winTitle != "AutoHotkey" && winTitle != "AutoHotkey64") {
                ; AutoHotkey 관련 접미사 제거
                winTitle := RegExReplace(winTitle, " - AutoHotkey.*$", "")
                
                ; 특수 형식으로 저장 (식별을 위해)
                currentProcess := "ahk_script:" . winTitle
            }
        }
...
GetAliasOrName(procKey) {
    ...
    ; "ahk_script:" 접두어로 시작하는 경우 - 오토핫키 스크립트 타이틀 반환
    if (SubStr(procKey, 1, 11) = "ahk_script:") {
        scriptTitle := SubStr(procKey, 12)
        return scriptTitle ; 스크립트 제목 그대로 표시
    }
