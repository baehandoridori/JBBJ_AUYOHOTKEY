...
Gui, Add, Tab, x12 y69 w330 h160 vBasicTab, 자동한영전환|파일경로 쉽게열기
...
; = [EDPS] 버튼
Gui, Add, Button, x360 y185 w80 h22 hwndHEDPS vBtnEDPS gToggleEDPS +%BS_PUSHLIKE%, 쉬운 폴더트리
...
ToggleEDPS:
{
    global checkEDPS, EDPSPID
    checkEDPS := !checkEDPS

    if (checkEDPS) {
        GuiControl, +Background00FF00, BtnEDPS  ; 쉼표 위치 수정
        GuiControl,, BtnEDPS, ON 폴더트리
        Run, "G:\공유 드라이브\개인작업일지 모음\개인작업일지_배한솔\02_업무\프로젝트\07_오토핫키 한솔프로젝트\new\EDPS\ExplorerDialogPathSelector_1.3.1.exe",, EDPSPID
    } else {
        GuiControl, +BackgroundFF0000, BtnEDPS
        GuiControl,, BtnEDPS, OFF 폴더트리
        
        ; 프로세스 종료 방식 변경
        Process, Exist, ExplorerDialogPathSelector_1.3.1.exe
        if (ErrorLevel) {
            Process, Close, ExplorerDialogPathSelector_1.3.1.exe
        }
        EDPSPID := 0     
    }
}
...
!`::
   ...
   if (alwaysOnTopWindow = activeWindow)
   {
       ...
       SetTimer, RemoveAlwaysOnTopTooltip, -500
   }
   else
   {
